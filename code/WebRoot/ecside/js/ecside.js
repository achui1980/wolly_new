...
function $_IEGC() {
	...
	ECSideUtil_removeEvent(document.body, "click",
			ECSideUtil.ColmunMenu.doHideMe);
	ECSideUtil_removeEvent(window, "resize", ECSideUtil.ColmunMenu.doHideMe);
	ECSideUtil_removeEvent(window, "resize", ECSideUtil.resizeAllGird);
	ECSideUtil_removeEvent(window, "load", ECSideUtil.onWindowload);
	ECSideUtil.createGird = null;
	for (var girdId in ECSideList) {
		var grid = ECSideList[girdId];
		// grid.handleScroll = null;
		grid.resizeWaitinBar = null;
		grid.resizeHeader = null;
		ECSideUtil_removeEvent(grid.ECListBodyZone, "scroll", grid.handleScroll);
		ECSideUtil_removeEvent(window, "resize", grid.handleResize);
		ECSideUtil_removeEvent(grid.ECListBodyZone, "resize", grid.handleResize);

		grid.handleResize = null;
		grid.handleScroll = null;
		CollectGarbage();
	}
	var popup = new ECSPopup("nearPagesBar");
	popup.hideMe = null;
	popup.setShow = null;
	// printObject(popup);
	popup.tryHideMe = null;
	ECSideUtil_removeEvent(popup.coreElement, "mouseover", popup.setShow);
	ECSideUtil_removeEvent(popup.coreElement, "mouseout", popup.hideMe);
	popup = new ECSPopup("columnMenu");
	popup.tryHideMe = null;
	ECSideUtil_removeEvent(popup.coreElement, "mouseover", popup.setShow);
	ECSideUtil_removeEvent(popup.coreElement, "mouseout", popup.hideMe);
	// ECSideUtil_removeEvent(ECSide.ECListBodyZone, "scroll",
	// ECSide.handleScroll);
	// ECSideUtil_removeEvent(window, "resize", ECSide.handleResize);
	CollectGarbage();
}
...
var ECSideList = {};
var asynchronous = true;// 是否ajax是否采用异步方式
...
var ECSide = function(formid) {
	...
	Me.fillForm = function(originalRequest) {
		...
		/*
		 * Me.initList(); if (typeof(Me.onLoad)=="function"){ Me.onLoad(); }
		 */
	};
	...
	Me.insertCallBack = function(responseObj) {
		...
		ecsideObj.hideWaitingBar();
		// zan添加于10/15,为了在新建收款记录保存冲帐明细时暂时刷新
		if (tableId != 'finacerecvDetail' && tableId != 'finaceDealDetail') {
			ECSideUtil.reload(tableId);
		}
	};
...
ECSideUtil.reload = function(formid, pageno) {
	...
	// 刷新表格时,加载报价和订单的锁定价格复选框
	if (formid == 'orderDetail' || formid == 'priceDetail') {
		addCheckFlag();
	}
};
...
ECSideUtil.checkAll = function(checkcontrolObj, checkboxname, formid) {
	...
	if (!form.elements[checkboxname].length) {
		...
		// 导入时添加到隐藏的下拉框
		if (checkboxname == 'fireClickChk') {
			form.elements[checkboxname].parentNode.fireEvent('onclick');
		}
		// 样品档案
		if (checkboxname == 'eleClickChk') {
			insertToSelect(form.elements[checkboxname]);
			document.getElementById("reportDiv").style.top = 100;
			document.getElementById("reportDiv").style.left = 100;
	...
	for (i = 0; i < form.elements[checkboxname].length; i++) {
		if (!form.elements[checkboxname][i].disabled) {
			...
			// 导入时添加到隐藏的下拉框
			if (checkboxname == 'fireClickChk') {
				form.elements[checkboxname][i].parentNode.fireEvent('onclick');
			}
			// 样品档案
			if (checkboxname == 'eleClickChk') {
				insertToSelect(form.elements[checkboxname][i]);
			}
		}
	}
	// 样品档案
	...
	if (formid == 'orderDetail' || formid == 'priceDetail') {
		...
		for (var i = 0; i < lock.length; i++) {
			// 勾选复选框
			var hang = lock[i].parentNode.parentNode;
			var eleTypeidLv1 = getCellObj(hang, formid, 'eleTypeidLv1',
					false);
			ECSideUtil.updateEditCell(eleTypeidLv1);
			var value = 0;
			if (lock[i].checked == true) {
				value = 1;
			} else {
				value = 0;
			}
			var rdm = getCellObj(hang, formid, 'rdm', true);
			if(formid == 'orderDetail' ){
				DWREngine.setAsync(false);
				cotOrderService.updateMapValueByEleId(rdm, "checkFlag", value,
						function(res) {
						});
				DWREngine.setAsync(true);
			}else{
				DWREngine.setAsync(false);
				cotPriceService.updateMapValueByEleId(rdm, "checkFlag", value,
						function(res) {
						});
				DWREngine.setAsync(true);
			}
...
ECSideUtil.queryECForm = function(formid, parameter, asy) {
	...
	form[pageid].value = 1;
	ECSideUtil.noExport(formid);
	// ?????????? modify by jason 2007-08-17
	// modify by zy 2009-06-02 注释
	// form[formid+ECSideConstants.FILTER_PREFIX+ECSideConstants.ACTION].value=ECSideConstants.FILTER_ACTION;
...
ECSideUtil.ColmunMenu.getFilterItem = function(menuWidth, columnObj,
	...
	if (templateText.toLowerCase().indexOf("<select ") == 0) {
		...
		templateText = ECSideUtil.replaceAll(templateText, " disabled=\"",
				" tempdisabled=\"");
...
ECSideUtil.editCell = function(cellObj, formid, templateId) {
	...
	if (editType == "input") {
		cellObj.innerHTML = ECSideUtil.replaceAll(templateText, "value=\"\"",
				"value=\"" + value + "\"");
		var area = cellObj.getElementsByTagName("TEXTAREA")[0];
		if (typeof(area) != 'undefined') {
			cellObj.innerHTML = cellObj.innerHTML.replace('><', '>' + value
							+ '<');
		}

		if (formid == 'splitDetail') {
			// 用于排载的编辑页面双击事件
			var p = cellObj.getElementsByTagName("INPUT")[0].name;
			if (p == 'containerCount') {
				cellObj.innerHTML = ECSideUtil
						.replaceAll(
								...
								"value=\""
										+ value
										+ "\" onchange='containNumClick(this)' onblur='ECSideUtil.updateEditCell(this)'");
			}
			if (p == 'boxCount') {
				cellObj.innerHTML = ECSideUtil
						.replaceAll(
								...
								"value=\""
										+ value
										+ "\" onchange='boxNumClick(this)' onblur='ECSideUtil.updateEditCell(this)'");
			}
		}
...
ECSideUtil.delFromGird = function(buttonObj, formid, deleteFlags) {
	...
	for (var i = checkBoxList.length - 1; i >= 0; i--) {
		if (checkBoxList[i].checked) {
			...
			if (rowObj && ECSideUtil.hasClass(rowObj, "add")) {
	 ...
	 * 
	 * if (crow && ECSideUtil.hasClass(crow,"added")){ return; } if (crow &&
	 * ECSideUtil.hasClass(crow,"add")){ crow.parentNode.removeChild(crow);
	 * return; }
	 * 
	 * if ( (!deleteFlags || !checkBoxList ) && crow){ if
	 * (ECSideUtil.hasClass(crow,"del")){ ECSideUtil.removeClass(crow,"del");
	 * }else{ ECSideUtil.addClass(crow,"del"); } return; }
	 * 
	 * if (!checkBoxList){ return ; } if ( !
	 * ECSideUtil.isCollection(checkBoxList) ){ checkBoxList=[checkBoxList]; }
	 * for (var i=0;i<checkBoxList.length;i++){ if (checkBoxList[i].checked){
	 * var rowObj=checkBoxList[i].parentNode.parentNode; if
	 * (ECSideUtil.hasClass(rowObj,"del")){
	 * ECSideUtil.removeClass(rowObj,"del"); }else{
	 * ECSideUtil.addClass(rowObj,"del"); } //rowObj.className="del"; } }
	 */

};

// 用于在订单处理金额
ECSideUtil.delFromGirdOld = function(buttonObj, formid, deleteFlags) {
...
ECSideUtil.addToGird = function(buttonObj, templateId, formid) {
	...
	for (var i = 0; i < ecsideObj.columnNum; i++) {
		...
		var objInput = cells[i].getElementsByTagName("input")[0];
		if (objInput)
			cells[i].className = objInput.name;

		var objSel = cells[i].getElementsByTagName("select")[0];
		if (objSel)
			cells[i].className = objSel.name;
...
ECSideUtil.setAsyn = function(isAsyn) {
	asynchronous = isAsyn;
}
ECSideUtil.doAjaxUpdate = function(url, pars, callBack, formid) {
	...
	var myAjax = new Ajax.Request(url, {
				requestHeaders : ECSideConstants.AJAX_HEADER,
				method : "POST",
				asynchronous : asynchronous, // 是否异步，默认true
				parameters : pars,
				onComplete : callBack
			});
	if (asynchronous) {
...
/*
 * 
 * if(model.getTableHandler().getTable().isEditable().booleanValue()){ // if
 * (StringUtils.isBlank(column.getCellName())){ //
 * column.setCellName(column.getProperty()); // } if
 * (StringUtils.isNotBlank(column.getCellEdit())){ String
 * editEvent="ondblclick"; String[] cellEdits=cellEdit.split(","); String
 * type=cellEdits[0]; String template=null; if (cellEdits.length>1){
 * template=cellEdits[1]; } if(StringUtils.isBlank(template)){
 * template="ec_edit_template_"+type; } if (cellEdits.length>2){
 * editEvent=cellEdits[2]; } column.setEditEvent(editEvent);
