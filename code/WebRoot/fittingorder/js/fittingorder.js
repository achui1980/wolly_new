Ext.onReady(function() {
	...
	var ds = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : "cotfittingorder.do?method=query"
						}),
	...
	function deleteBatch() {
		...
		Ext.MessageBox.confirm('提示消息', "确定删除选中的配件采购单?", function(btn) {
					if (btn == 'yes') {
						cotFitOrderService.deleteFitOrderList(ary,
								function(res) {
									if (res == 3) {
...
function getIds() {
	var list = Ext.getCmp("fittingGrid").getSelectionModel().getSelections();
	var res = new Array();
	Ext.each(list, function(item) {
				var fittingOrder = new CotFittingOrder();
				fittingOrder.id = item.id;
				res.push(fittingOrder);
			});
	return res;
}
// 删除
function del(id, status) {
	...
	Ext.MessageBox.confirm('提示消息', "确定删除选中的配件采购单?", function(btn) {
				if (btn == 'yes') {
					cotFitOrderService.deleteFitOrderList(ids, function(res) {
								if (res == 3) {
									Ext.Msg.alert("提示信息", "删除出错!");
									return;
								}
								if (res == 1) {
									reloadGrid('fittingGrid');
									Ext.Msg.alert("提示信息", "该配件采购单含有应付账款,不能删除!");
									return;
								}
								if (res == 2) {
									reloadGrid('fittingGrid');
									Ext.Msg.alert("提示信息",
											"该配件采购单有加/减费用导入到出货,不能删除!");
									return;
								}
								if (res == 0) {
									reloadGrid('fittingGrid');
