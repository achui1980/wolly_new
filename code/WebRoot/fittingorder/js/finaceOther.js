Ext.onReady(function() {
	...
	var otherCurrencyId = ""
	cotFitOrderService.getList('CotPriceCfg', function(res) {
				if (res.length != 0) {
	...
	var otherfeeds = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
					api : {
						read : "cotfittingorder.do?method=queryFinanceOther&fkId="
								+ parent.$('pId').value,
						create : "cotfittingorder.do?method=addOther",
						update : "cotfittingorder.do?method=modifyOther"
					},
					listeners : {
						// 保存表格前显示提示消息
						...
						exception : function(proxy, type, action, options, res,
							...
							if (res.status != 200) {
								...
								otherfeeds.reload({params : {
									start : 0,
									limit : 15
								}});
								// 修改主单的实际金额
								cotFitOrderService.modifyRealMoney(parent
	...
	var dealds = new Ext.data.Store({
				...
				method : 'post',
				baseParams : {
					orderNo : parent.$('fittingOrderNo').value,
				...
				proxy : new Ext.data.HttpProxy({
							api : {
								read : "cotfittingorder.do?method=queryDeal"
							}
						}),
				reader : new Ext.data.JsonReader({
							root : "data",
	...
	var detailds = new Ext.data.Store({
				...
				method : 'post',
				proxy : new Ext.data.HttpProxy({
							url : "cotfittingorder.do?method=queryDealDetail"
						}),
				reader : new Ext.data.JsonReader({
							root : "data",
							totalProperty : "totalCount",
	...
	otherfeegrid.on("celldblclick", function(grid, rowIndex, columnIndex, e) {
				...
				if (dataIndex=='flag' && rec.data.amount!=rec.data.remainAmount) {
					return false;
				}
				// 获得view
					...
					if (e.value == "") {
						...
						if (flag == false) {
							...
							if (id != "" && id != "null") {
								...
								// 判断是否重复
								cotFitOrderService.findIsExistName(e.value, id,
										recId, function(res) {
											if (res == true) {
		...
		if (idsAry.length > 0) {
			Ext.MessageBox.confirm('提示消息',
					'您是否确定删除选择的费用?已生成应付帐或者该费用有导入出货,不能删除!', function(btn) {
						if (btn == 'yes') {
							cotFitOrderService.deleteByIds(idsAry,
									function(res) {
										if (res != null) {
											// 修改主单的实际金额
	...
	function onDelDeal() {
		...
		var str = "";
		// 查询应付帐是否有导到出货
		cotFitOrderService.checkIsImport(idsAry, function(newIds) {
					if (newIds.length > 0) {
						var str = '';
						...
						var flag = Ext.Msg.confirm('提示消息', str, function(btn) {
									if (btn == 'yes') {
										cotFitOrderService.deleteByAccount(newIds,
												function(res) {
													otherfeeds.reload({params : {
														start : 0,
														limit : 15
			...
			if (btn == 'yes') {
				...
				var task = new Ext.util.DelayedTask(function() {
					...
					if (ids != '') {
						// 修改所有其他费用的status为1
						DWREngine.setAsync(false);
						cotFitOrderService.updateStatus(ids, function(res) {
									otherfeeds.reload();
									// 重新设置dealds的查询路径
									dealds.proxy.setApi({
												read : "cotfittingorder.do?method=queryDeal&fkId="
														+ parent.$('pId').value
											});
									dealds.reload();
								});
	...
	function addByOtherMoney(id, name, money, currencyId) {
		...
		detail.fkId = mainId;
		var orderNo = parent.$('fittingOrderNo').value;
		var factoryId = parent.$('factoryId').value;
		var busId = parent.$('empId').value;
		// 生成应付帐款单号
		DWREngine.setAsync(false);
		cotFitOrderService.createDealNo(parseInt(factoryId), function(res) {
					detail.finaceNo = res;
					...
					// 保存应付帐款
					cotFitOrderService.saveAccountdeal(detail, parent
	...
	this.saveOther = function(mainId, orderNo, source, type, factoryId) {
		...
		otherfeeds.proxy.setApi({
					read : "cotfittingorder.do?method=queryFinanceOther&fkId="
							+ parent.$('pId').value,
					create : "cotfittingorder.do?method=addOther" + urlAdd,
					update : "cotfittingorder.do?method=modifyOther" + urlMod
				});
		otherfeeds.save();
	}
