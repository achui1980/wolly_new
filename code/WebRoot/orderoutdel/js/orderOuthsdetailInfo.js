...
window.onunload = function() {
	cotOrderOutService.clearMap("chaDel", function(res) {
			});
}
// 获取报价和订单的单价要保留几位小数
...
Ext.onReady(function() {
	...
	var ds = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			api : {
				read : "cotorderout.do?method=queryChaInfoDel&orderId="
						+ $('orderId').value,
			...
			listeners : {
				...
				load : function(store, recs, options) {
					options.callback = function(r, ops, success) {
						if (success == false) {
							Ext.Msg.alert("Message", "数据查询错误!请联系管理员!");
						} else {
...
//						}, '-', 
	...
	function calPrice() {
		...
		cotOrderOutService.calPriceAll(rdm, function(res) {
					if (res != null) {
						...
						Ext.MessageBox.alert('Message', '请先选中一条明细!');
					}
				});
	}
	...
	function initForm(record) {
		...
		DWREngine.setAsync(false);
		cotOrderOutService.getChaOrderDelMapValue(rdm, function(res) {
			if (res != null) {
				if (res.boxCbm != null) {
				...
				if (popdom == true) {
					...
					if (!flag) {
						...
						res.picPath = "./showPicture.action?flag=chaDel&detailId="
								+ record.id + "&tmp=" + p;
					}
				}
	...
	function insertAll() {
		if (parent.$('orderStatus').value == 1
			...
			Ext.MessageBox.alert('Message', '对不起,该单已经被审核不能再修改!若要更改请反审!');
			return;
		}
		...
		// 如果表格有数据清空数据
		if (ds.getCount() > 0) {
			Ext.MessageBox.confirm('提示信息', "导入全部出货明细前,需要先清空报关明细数据,是否继续?",
					function(btn) {
						if (btn == 'yes') {
							DWREngine.setAsync(false);
							mask();
	...
	function insertUpdate() {
		...
		if (mods.length == 0) {
			Ext.MessageBox.alert('Message', '目前出货明细没有最新记录,旧记录也没被修改过,不能导入!');
			unmask();
			return;
		}
	...
	function showModel(item, pressed) {
		...
		var cord = sm.getSelections();
		if (cord.length == 0) {
			Ext.MessageBox.alert("Message", "请先勾选需要修改报关品名的明细!");
			return;
		}
		if ($('importDiv') == null) {
	...
	function showCe(item, pressed) {
		...
		var cord = sm.getSelections();
		if (cord.length == 0) {
			Ext.MessageBox.alert("Message", "请先勾选报关明细!");
			return;
		}
		if (cePanel == null) {
	...
	function sumSortTable() {
		...
		cotOrderOutService.updateSortNoBao(type, sort.field, fieldType,
				function(res) {
					if (res) {
						ds.reload();
						Ext.MessageBox.alert('Message', "保存表格排序成功!");
					} else {
						Ext.MessageBox.alert('Message', "保存表格排序失败!");
					}
				});
		DWREngine.setAsync(true);
	}
