Ext.onReady(function() {
	...
	grid.on('rowclick', function(grid, rowIndex, event) {
				...
				if(!Ext.isEmpty(record.data.faName)){
					Ext.getCmp('faName').setValue(record.data.faName.replace(/,/gi,'\r\n'));
				}
				if(!Ext.isEmpty(record.data.baoName)){
					Ext.getCmp('baoName').setValue(record.data.baoName.replace(/,/gi,'\r\n'));
				}
	...
	function save() {
		...
		if(!Ext.isEmpty(formData.faName)){
			formData.faName = formData.faName.trim().replace(/\n/g,',');
			formData.faName = formData.faName.trim().replace(/\r\n/g,',');
		}
		if(!Ext.isEmpty(formData.baoName)){
			formData.baoName = formData.baoName.trim().replace(/\n/g,',');
			formData.baoName = formData.baoName.trim().replace(/\r\n/g,',');
		}
		// 判断编号是否重复
		cotTypeLvService.findExistByEnNo(formData.typeEnName, formData.id,
				function(res) {
					if (res != null) {
						Ext.MessageBox.show({
									title : 'Message',
						...
						cotTypeLvService.saveOrUpdate(formData,
												function(res) {
													if (res) {
														clearForm("addFormId");
														reloadGrid("typeGrid");
	...
	function deleteBatch() {
		...
		Ext.MessageBox.confirm('Message', 'Are you sure to delete this record?', function(btn) {
					if (btn == 'yes') {
						cotTypeLvService.deleteByIds(list, function(res) {
									clearForm("addFormId");
									ds.reload();
...
function del(id) {
	...
	Ext.MessageBox.confirm('Message', 'Are you sure to delete this record?', function(btn) {
				if (btn == 'yes') {
					var list = new Array();
					list.push(id);
					cotTypeLvService.deleteByIds(list, function(delres) {
								if (delres) {
									clearForm("addFormId");
									reloadGrid("typeGrid");
