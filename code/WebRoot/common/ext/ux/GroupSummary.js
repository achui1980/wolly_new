...
//        var t = data[colName].add(Ext.num(v, 0)), count = record.store.getCount();
        var t = Number(data[colName]).add(Ext.num(v, 0));
        var totalCount = record.store.getCount();
        var count = 0;
        record.store.each(function(item){
        	if(item.data[colName]!=0){
        		count++;
        	}
        });
        if(count==0){
        ...
        return rowIdx == totalCount - 1 ? ((t/count).toFixed("2")) : t;
    }
}