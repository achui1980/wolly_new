/*
 * My97 日期控件 My97 DatePicker Ver 2.1 Build:20070122 Blog:
 * http://blog.csdn.net/my97/ Mail: smallcarrot@163.com
 */
/* 设置 */
var dpcfg = {};
...
/* 语言设置 */
dpcfg.aWeekStr = ["SUN", "MON", "THU", "WED", "THU", "FIR", "SAT"];
dpcfg.aMonStr = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct",
		"Nov", "Dec"];
dpcfg.todayStr = "Today";
dpcfg.okStr = "OK";
dpcfg.timeStr = "Time";
dpcfg.monthStr = "Month";
dpcfg.yearStr = "Year";
...
function WdatePicker(el, dateFmt, showTime, skin) {
	this.win = window;
	this.top = window;
	while (this.top.parent.document != this.top.document
			&& this.top.parent.document.getElementsByTagName("frameset").length == 0) {
	...
	if (c.substring(0, 1) == '$') {
		...
		if (c) {
			if (c.getAttribute("REALVALUE") == null
					|| c.getAttribute("REALVALUE") == "") {
				c = c.value;
			} else {
	...
	if (this.top.document.dateDiv
			&& this.top.document.dateDiv.obj.eCont == this.eCont) {
		$d = this.top.document.dateDiv;
	} else {
		this.top.document.dateDiv = null;
	}
	this.cssPath = 'common/css/datepicker.css';
	...
	if ((bd + parseInt(this.dd.offsetHeight) < (ar.height))
			|| (bd - this.eCont.offsetHeight < this.dd.offsetWidth * 0.8)) {
		this.dd.style.top = (this.top.document.body.scrollTop + bd + 1) + 'px';
	} else {
		this.dd.style.top = (this.top.document.body.scrollTop + bd
				- parseInt(this.dd.offsetHeight) - this.eCont.offsetHeight - 3)
				+ 'px';
	}
	this.dd.style.left = -1 + this.top.document.body.scrollLeft
			+ Math.min(br, ar.width - parseInt(this.dd.offsetWidth) - 5) + 'px';
	this.dd.ifr.style.cssText = 'top:'
			+ this.dd.style.top
			+ ';left:'
			+ (this.dd.style.left)
			+ ';width:'
			+ Math.min(180, (this.dd.offsetWidth - 1))
			+ 'px;height:'
			+ (this.dd.offsetHeight - 1)
			+ 'px;position:absolute;z-index:196;overflow:hidden;border:0px;filter:alpha(opacity=0);';
}
...
WdatePicker.prototype.bo = function() {
	if (!$d || ($d && $d.obj.skin != this.skin)) {
		...
		for (var i = this.top.document.styleSheets.length - 1; i >= 0; i--) {
			...
			if (tempStyle.href.substring(tempStyle.href.lastIndexOf('/') + 1)
					.toLowerCase() == 'datepicker.css') {
...
WdatePicker.prototype._fillmonth = function() {
	...
	for (i = 0; i < 6; i++) {
		s.a("<tr><td ");
		ac = (ax)
				|| (!mmmy97 && ((af && t[i] >= this.minDate.month) || (ae && t[i] <= this.maxDate.month)))
				|| (mmmy97 && (t[i] >= this.minDate.month && t[i] <= this.maxDate.month));
		s
				.a((ac)
						? "class='Wym' onmouseover=\"this.className='WdayOn'\" onmouseout=\"this.className='Wym'\" onmousedown=\"$d.mInput.value='"
								+ t[i]
		...
		s.a("<td ");
		ac = (ax)
				|| (!mmmy97 && ((af && t[i + 6] >= this.minDate.month) || (ae && t[i
						+ 6] <= this.maxDate.month)))
				|| (mmmy97 && (t[i + 6] >= this.minDate.month && t[i + 6] <= this.maxDate.month));
		s
				.a((ac)
						? "class='Wym' onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown=\"$d.mInput.value='"
								+ t[i + 6]
								+ "';$d.mDiv.style.display='none';$d.mInput.blur();\""
						: "class='Winvalidym'");
		s.a(">" + at[i + 6] + "</td></tr>");
	...
	s
			.a("<td align=center onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown=\"$d.mDiv.style.display='none';\">×</td</tr>");
	s.a("</table>");
...
WdatePicker.prototype._fillyear = function(ao, au) {
	...
	for (i = 0; i < n; i++) {
		...
		s
				.a((ah)
						? "class='Wym' onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown=\"$d.yInput.value='"
								+ a[i]
	...
	s
			.a((this.minDate.year < ao)
					? "class='Wym' onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown='$d.obj._fillyear("
							+ (ao - 10) + "," + (au - 10) + ")'"
					: "class='Winvalidym'");
	s
			.a(">←</td><td class='Wym' onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown=\"$d.yDiv.style.display='none';$d.yInput.blur();\">×</td><td ");
	s
			.a((this.maxDate.year > au)
					? "class='Wym' onmouseover=\"this.className='WymOn'\" onmouseout=\"this.className='Wym'\" onmousedown='$d.obj._fillyear("
							+ (ao + 10) + "," + (au + 10) + ")'"
					: "class='Winvalidym'");
	s.a(">→</td></tr></table>");
...
WdatePicker.prototype._returnDateStr = function(Y, M, D, h, m, s, fmt) {
	...
	var ab = fmt.replace(/%[Yy]/, this.ad(Y, 4)).replace(/%[M]/, this.ad(M, 2))
			.replace(/%[Dd]/, this.ad(D, 2));
	if (this.showTime) {
		ab = ab.replace(/%[Hh]/, this.ad(h, 2)).replace(/%[m]/, this.ad(m, 2))
				.replace(/%[Ss]/, this.ad(s, 2));
	}
	return ab;
};
...
WdatePicker.prototype.aq = function() {
	...
	s.a("<div style='float:left;margin:2px'><div class='ymsel'></div>"
			+ dpcfg.monthStr
			+ "<input class='yminput' style='width:40px;' maxlength=2 value="
			+ this.month + "></div>");
	s.a("<div style='float:right;margin:2px'><div class='ymsel'></div>"
			+ dpcfg.yearStr
			+ "<input class='yminput' style='width:50px;' maxlength=4 value="
			+ this.year + "></div></div>");
	s.a("<div></div>");
	s.a("<div>");
	s.a(this.av());
...
WdatePicker.prototype.av = function() {
	...
	s
			.a("<div id=dpTime style='"
					...
					+ this.timeStr + "</span>");
	s.a(" <input class=tB maxlength=2 value=" + this.hour
			+ "><input value=':' class=tm readonly>");
	s.a("<input class=tE maxlength=2 value=" + this.minute
			+ "><input value=':' class=tm readonly>");
	s.a("<input class=tE maxlength=2 value=" + this.sec + "></td><td>");
	s
			.a("<button id=dpTimeUp></button></td></tr><tr><td><button id=dpTimeDown></button></td></tr></table></div>");
	s.a("<div style='float:right;margin-top:3px;text-align:right'>");
	s
			.a("<input id=dpOkInput type=button style='height:20px;width:90%'></input>");
	s.a("</div>");
	return s.j();
};
WdatePicker.prototype._setOkInput = function() {
	var d = $d.obj;
	if (d.eCont.value == "") {
		if (((this.aa * 10000 + this.p * 100 + this.z * 1) >= (this.minDate.year
				* 10000 + this.minDate.month * 100 + this.minDate.date * 1))
				&& ((this.aa * 10000 + this.p * 100 + this.z * 1) <= (this.maxDate.year
						* 10000 + this.maxDate.month * 100 + this.maxDate.date
						* 1))) {
		...
		$d.okInput.value = d.todayStr
				+ ((d.showTime) ? "" : (" " + d.ad(d.aa, 4) + '-'
						+ d.ad(d.p, 2) + '-' + d.ad(d.z, 2)));
	} else {
		$d.okInput.onclick = function() {
			$d.obj.pickDate();
		};
...
WdatePicker.prototype.u = function() {
	var e, tempMonth;
	if ((this.year * 100 + this.month * 1) < (this.minDate.year * 100 + this.minDate.month
			* 1)) {
		this.dd.yInput.value = e = this.minDate.year;
		this.dd.mInput.value = tempMonth = this.minDate.month;
	} else if ((this.year * 100 + this.month * 1) > (this.maxDate.year * 100 + this.maxDate.month
			* 1)) {
	...
	s
			.a("<table id=dpDayTable width=100% border=0 cellspacing=0 cellpadding=0>");
		...
		for (k = 0; k < 7; k++) {
			if (j >= 1 && j <= lastDate) {
				if (bk && (j == this.bs)) {
					...
					ag = ((this.highLineWeekDay && (k == 0 || k == 6))
							? 'Wwday'
							: 'Wday');
				}
				classOnStr = ((this.highLineWeekDay && (k == 0 || k == 6))
						? 'WwdayOn'
						: 'WdayOn');
				s.a("<td align=center ");
				if (bh
						|| (!ccmy97 && ((aj && j >= this.minDate.date) || (ai && j <= this.maxDate.date)))
						|| (ccmy97 && (j >= this.minDate.date) && (j <= this.maxDate.date))) {
	...
	HTMLElement.prototype.getBoundingClientRect = function() {
		...
		while (obj = obj.offsetParent) {
			if (obj.style.position == 'absolute'
					...
					|| (obj.style.overflow != 'visible' && obj.style.overflow != '')) {
				break;
			}
			top += obj.offsetTop;
			left += obj.offsetLeft;
...
function disposeDatePicker() {
	if ($d != undefined && $d.obj != undefined
			&& $d.obj.eCont != window.event.srcElement
