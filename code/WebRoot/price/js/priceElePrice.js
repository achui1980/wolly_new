...
function validPrice(obj){
	...
	if(parseFloat(obj.value)>=1000){
		alert('Cost price can not exceed 1000!');
		obj.value=0;
	}
...
function addNewGrid() {
	if (parent.parent.$('priceStatus').value == 2 && loginEmpId != "admin") {
		alert('Sorry, this one has the additional cost can not be audited!');
		return;
	}
...
function deleteSelectRow() {
	if (parent.parent.$('priceStatus').value == 2 && loginEmpId != "admin") {
		alert('Sorry, this one has been unable to delete the cost of audit!');
		return;
	}
	var isPopedom = getPdmByOtherUrl("cotprice.do", "MOD");
	if (isPopedom == 0) {// 没有删除权限
		alert("You do not have permission to offer amendments to");
		return;
	}
...
function save() {
	if (parent.parent.$('priceStatus').value == 2 && loginEmpId != "admin") {
		alert('Sorry, this one has been the cost of audit can not be saved!');
		return;
	}
	var isPopedom = getPdmByOtherUrl("cotprice.do", "MOD");
	if (isPopedom == 0) {// 没有删除权限
