...
public class UploadOrderScServlet extends HttpServlet implements Servlet {

	private static final long serialVersionUID = 946573126457064359L;
	private static final String DOC_DIR = "/reportfile/orderSc";
	Logger logger = Log4WebUtil.getLogger(UploadOrderStatusFileServlet.class);
	
	public UploadOrderScServlet() {
		super();
	}
			...
			if (items!=null){
				for(int i=0;i<items.size();i++){
					...
					if (fileItem.getName()!=null && fileItem.getName().length()>0){
						...
						if(gt.equals("1")){
							fileName = "SC-"+orderNo+"."+fx;
						}else{
							fileName = "Client-"+orderNo+"."+fx;
						}
