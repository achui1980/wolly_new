...
样品编辑页面上传图片
 */
public class UploadElePictureServlet extends HttpServlet implements Servlet {
	...
	public static final int MAX_EACH_FILE_UPLOAD_SIZE = 1024000;
	Logger logger = Log4WebUtil.getLogger(UploadElePictureServlet.class);
	/**
	 * Constructor of the object.
	 */
	public UploadElePictureServlet() {
		super();
	}
					...
					for(int i=0;i<items.size();i++){
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							if(pEId!=null && !"".equals(pEId)){
								...
								try {
									cotElementsService.updateDefaultPic(fPath, Integer.parseInt(pEId));
									//保存到系统日记表
									CotSyslog cotSyslog = new CotSyslog();
									cotSyslog.setEmpId(empId);
									cotSyslog.setOpMessage("更改样品主图片成功");
									...
									CotSyslog cotSyslog = new CotSyslog();
									cotSyslog.setEmpId(empId);
									cotSyslog.setOpMessage("更改样品主图片失败,失败原因:"+e.getMessage());
									cotSyslog.setOpModule("elements");
									cotSyslog.setOpTime(new Date(System.currentTimeMillis()));
									cotSyslog.setItemNo(pEId);
									cotSyslog.setOpType(2);
