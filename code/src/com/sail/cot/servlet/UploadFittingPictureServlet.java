 ...
 * 配件编辑页面上传图片
 */
public class UploadFittingPictureServlet extends HttpServlet implements Servlet {
	...
	Logger logger = Log4WebUtil.getLogger(UploadElePictureServlet.class);
	/**
	 * Constructor of the object.
	 */
	public UploadFittingPictureServlet() {
		super();
	}
				...
				if (items!=null){
					...
					for(int i=0;i<items.size();i++){
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							if(pEId!=null && !"".equals(pEId) && !"null".equals(pEId)){
								//保存到数据库(更新图片表和样品表)
								CotFittingsService cotFittingsService = (CotFittingsService)this.getService("CotFittingsService");
								CotSysLogService sysLogService = (CotSysLogService)this.getService("CotSysLogService");
								try {
									cotFittingsService.updateDefaultPic(fPath, Integer.parseInt(pEId));
