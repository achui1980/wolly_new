...
public class UploadCustPcServlet extends HttpServlet implements Servlet {

	private static final long serialVersionUID = 946573126457064359L;
	private static final String DOC_DIR = "/artWorkPdf/client";
	Logger logger = Log4WebUtil.getLogger(UploadCustPcServlet.class);
	
	public UploadCustPcServlet() {
		super();
	}
			...
			if (items!=null){
				for(int i=0;i<items.size();i++){
					...
					if (fileItem.getName()!=null && fileItem.getName().length()>0){
						...
						fileName = name+"_"+org.apache.commons.lang.RandomStringUtils.randomNumeric(8)+"."+fx;
						if(fx.toLowerCase().equals("pdf")){
							...
							json.put("msg", "Upload format error, File id not a pdf!");
						}
					}
				}
			}
