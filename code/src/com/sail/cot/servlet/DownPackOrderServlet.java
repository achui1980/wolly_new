...
public class DownPackOrderServlet extends javax.servlet.http.HttpServlet
	...
	private void downFile(HttpServletRequest request,
		...
		String gId = request.getParameter("packorderId");// 主包材采购单
		if (gId != null && !gId.trim().equals("")) {
			queryString.append(" and obj.packing_orderId=" + gId);
		}else{
			return;
		}
		...
		if ("XLS".equals(printType)) {
			...
			this.setFileDownloadResponseHeader(response, packingNo+".xls");
		...
		if ("PDF".equals(printType)) {
			...
			this.setFileDownloadResponseHeader(response, packingNo+".pdf");
			exporter
					.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
