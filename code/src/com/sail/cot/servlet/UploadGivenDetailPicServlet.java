...
public class UploadGivenDetailPicServlet extends HttpServlet implements Servlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 946573126457064359L;
	private static final String DOC_DIR = "/givenImg";
	Logger logger = Log4WebUtil.getLogger(UploadGivenDetailPicServlet.class);
	/**
	 * Constructor of the object.
	 */
	public UploadGivenDetailPicServlet() {
		super();
	}
		...
		if (request.getContentLength()>Constant.MAX_ONCE_UPLOAD_SIZE){
			...
			try {
				...
				if (items!=null){
					...
					String givenNo = "/"+(String) map.get("givenNo");
					
					String picName = (String) map.get("picName")+".png";
					/*//删除原来的图片
					File fileParent = new File(request.getRealPath(DOC_DIR)+givenNo);
					...
					for(int i=0;i<items.size();i++){
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							//如果givenImg文件夹不存在就新建
							File givenImg = new File(request.getRealPath(DOC_DIR)+givenNo);
							if(!givenImg.exists()){
								givenImg.mkdirs();
							}
							String uploadPath = request.getRealPath(DOC_DIR)+givenNo+File.separator+picName;
							File uploadedFile = new File(uploadPath);
							fileItem.write(uploadedFile); 
							
							//保存到数据库(更新图片表和样品表)
							CotGivenService cotGivenService = (CotGivenService)this.getService("CotGivenService");
							String detailId=(String) map.get("detailId");
							cotGivenService.updatePicImg(request,uploadPath, Integer.parseInt(detailId));
