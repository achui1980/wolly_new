 ...
 * 订单产品标上传
 */
public class UploadProductMBPicServlet extends HttpServlet implements Servlet {
	...
	Logger logger = Log4WebUtil.getLogger(UploadProductMBPicServlet.class);

	public UploadProductMBPicServlet() {
		super();
	}
					...
					for (int i = 0; i < items.size(); i++) {
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							if("1".equals(mbType)){
								...
								cotOrderFacService.updateProMBImg(uploadPath, Integer.parseInt(mainId));
							}else{
								CotOrderService cotOrderService = (CotOrderService)this.getService("CotOrderService");
								cotOrderService.updateProMBImg(uploadPath, Integer.parseInt(mainId));
							}
