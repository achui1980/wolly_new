 ...
 * 订单麦标上传
 */
public class UploadPictureServlet extends HttpServlet implements Servlet {
	...
	Logger logger = Log4WebUtil.getLogger(UploadPictureServlet.class);
					...
					for (int i = 0; i < items.size(); i++) {
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							if(mainId!=null && !"".equals(mainId)){
								...
								cotOrderOutService.updateMBImg(uploadPath, Integer.parseInt(mainId));
							}
							
							//保存采购唛标图片到数据库
							String orderfacId=(String) map.get("orderfacId");
							if(orderfacId!=null && !"".equals(orderfacId)){
								CotOrderFacService cotOrderFacService = (CotOrderFacService)this.getService("CotOrderFacService");
								cotOrderFacService.updateMBImg(uploadPath, Integer.parseInt(orderfacId),request);
							}
