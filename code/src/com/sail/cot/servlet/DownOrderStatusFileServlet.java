 ...
 *
 */
public class DownOrderStatusFileServlet extends HttpServlet {
	...
	private void downFile(HttpServletRequest request,
		...
		CotBaseDao baseDao = (CotBaseDao)getService("CotBaseDao");
		String fileId = request.getParameter("id");
		CotOrderstatusFile orderstatusFile = (CotOrderstatusFile)baseDao.getById(CotOrderstatusFile.class, Integer.valueOf(fileId));
		String fileUrl = orderstatusFile.getFilePath();
		byte[] buf = new byte[1024];
		int b = 0;
		if(fileUrl != null){
			File file = new File(ContextUtil.getRealPath()+"/"+fileUrl);
			String fileName= orderstatusFile.getFileName();
