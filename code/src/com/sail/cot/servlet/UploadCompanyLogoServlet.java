...
public class UploadCompanyLogoServlet extends HttpServlet implements Servlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 946573126457064359L;
	private static final String DOC_DIR = "/companyLogo";
	Logger logger = Log4WebUtil.getLogger(UploadCompanyLogoServlet.class);
	/**
	 * Constructor of the object.
	 */
	public UploadCompanyLogoServlet() {
		super();
	}
					...
					for(int i=0;i<items.size();i++){
						...
						if (fileItem.getName()!=null && fileItem.getName().length()>0){
							...
							if(companyId!=null && !"".equals(companyId) && !"null".equals(companyId)){
								...
								try {
									cotCompanyService.updatePicImg(fPath, Integer.parseInt(companyId));
