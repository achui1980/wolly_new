...
public class DownLabelRptServlet extends javax.servlet.http.HttpServlet
	...
	private void downFile(HttpServletRequest request,
		...
		if (queryAgain == null) {
			String labelId = request.getParameter("labelId");// 主报价单号
		...
		if ("XLS".equals(printType)) {
			...
			this.setFileDownloadResponseHeader(response, "标签单.xls");
		...
		if ("PDF".equals(printType)) {
			...
			this.setFileDownloadResponseHeader(response, "标签单.pdf");
			exporter
					.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
			exporter.setParameter(JRExporterParameter.OUTPUT_STREAM,
					ouputStream);
		...
		if ("HTML".equals(printType)) {
			...
			this.setFileDownloadResponseHeader(response, "标签单.html");
			exporter
					.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
