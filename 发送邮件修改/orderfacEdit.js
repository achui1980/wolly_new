...
Ext.onReady(function() {
	...
	baseDataUtil.getBaseDicDataMap("CotConsignCompany", "id", "companyRemark", function(
					res) {
		...
		listeners:{
			...
			'afterrender' : function(area) {
							if ($('pId').value != ''&& $('pId').value != 'null') {
								...
								companyCodeBox.bindPageValue('CotConsignCompany','id',txtAreaAry.forwardingAgent);
							}
						}
		}
	});
	...
	var orderStatusStore = new Ext.data.SimpleStore({
				fields : ["id", "name"],
				data : [[0, 'Waiting'], [1, 'No Pass'], [2, 'Approved'], [3, 'Approval'],
						[9, 'No Audit']]
			});
	...
	var empShenBox = new BindCombox({
		dataUrl : "servlet/DataSelvert?tbname=CotEmps&key=empsName&typeName=shenFlag&type=1&logId="+logId,
		cmpId : 'shenPerson',
		fieldLabel : "Audit",
		editable : true,
		flex : 2,
	...
	var cm = new HideColumnModel({
				...
				columns : [
						sm,// 添加复选框列
						{
							...
							header : "<font color=blue>Purchase Price</font>",
							dataIndex : "priceFac",
							editable:true,
							editor : new Ext.form.NumberField({
	...
	var timeForm = new Ext.form.FormPanel({
		...
		items : [{
					...
					items : [{
								...
								items : [{
											xtype : "cdatefield",
											fieldLabel : "Artwork sent to supplier",
											id : "facDeadline",
											// value:new Date(),
											format : 'Y-m-d',
											anchor : "100%",
											...
											xtype : "cdatefield",
											fieldLabel : "Artwork sent to client",
											id : "pcaketDeadline",
											// value:new Date(),
											format : 'Y-m-d',
											anchor : "100%",
		...
		cotOrderFacService.findIsExistInOrder(key, function(res) {
			...
			if (same.length != 0) {
				Ext.MessageBox.confirm('Message', 'Existing Order Details'
								+ sameTemp
								+ "Overwrite the original order details?",
						function(btn) {
							if (btn == 'yes') {
	...
	var orderfacForm = new Ext.form.FormPanel({
		...
		listeners : {
			...
			'expand' : function(pnl) {
				Ext.Element.fly("talPriceDiv").setLeftTop(420, 173);
			}
		},
		items : [{
			xtype : "panel",
							...
							'check' : function(check, bo) {
								...
								if (bo == true) {
									...
									if (Ext.isEmpty(facId)) {
										...
										$(buyer).value = 'Wolly & Co,\n'
												+ 'Hovedgaden 2,\n'
												+ '4330 Hvalsø,\n' + 'Denmark';
										txtAreaAry.buyer='Wolly & Co,\n'
												+ 'Hovedgaden 2,\n'
												+ '4330 Hvalsø,\n' + 'Denmark';		
		...
		if (id != 'null' && id != '') {
			...
			cotOrderFacService.getOrderFacById(parseInt(id), function(res) {
						...
						if (res.totalMoney != null) {
							$('totalLab').innerText = res.totalMoney
									.toFixed("2");
						} else {
...
//						txtAreaAry.booking=res.booking;
						...
						if ((res.orderStatus == 0 || res.orderStatus == 2) && (loginEmpId == "admin" || logId!=res.checkPerson)) {
							Ext.getCmp("qingBtn").show();
						}
						
						cotOrderFacService.getCanOutByOrderId(res.orderId,function(ck){
							// 出货填过单号后,隐藏保存按钮
							if (ck == 2) {
...
//					.alert(
//							'Message',
//							'Sorry, this one has been reviewed, and if edit! Please review again or contact the webmaster!');
//			return;
//		}
		...
		Ext.MessageBox.confirm('Message',
				'Do you want to save the purchase order？', function(btn) {
					if (btn == 'yes') {
						...
						if ($('pId').value != 'null' && $('pId').value != '') {
							...
							for (var p in form) {
								if (p != 'orderTime' && p != 'sendTime' && p != 'shipmentDate'
													&& p != 'addTime'
													&& p != 'modTime') {
						...
						if (Ext.getCmp("orderClause")
								&& Ext.getCmp("orderClause").isVisible()) {
						...
						// {
						// cotOrderFac.checkReason = $('checkReason').value;//
						// 审核理由
						// }
						// 其它信息
						//if (Ext.getCmp("quality").isVisible()) {
						   ...
						   if(Ext.isEmpty(Ext.getCmp('saleUnit').getValue())){
						    ...
						    if(Ext.isEmpty(Ext.getCmp('seller').getValue())){
						    	...
						    	cotOrderFac.seller =Ext.getCmp('seller').getValue();
						    }
						    // TODO:
							if(Ext.isEmpty(Ext.getCmp('agent').getValue())){
						    	cotOrderFac.agent =txtAreaAry.agent;
						    }else{
						    	cotOrderFac.agent = Ext.getCmp('agent').getValue();
						    }
						    //1表示选中
							...
							if(Ext.isEmpty(Ext.getCmp('forwardingAgent').getValue())){
						    	cotOrderFac.forwardingAgent=txtAreaAry.forwardingAgent;
						    }else{
						    	cotOrderFac.forwardingAgent = Ext.getCmp('forwardingAgent').getValue();
						    }
						    
						    if(Ext.isEmpty(Ext.getCmp('booking').getValue())){
						    	cotOrderFac.booking =txtAreaAry.booking;
						    }else{
						    	cotOrderFac.booking = Ext.getCmp('booking').getValue();
						    }
						    if(Ext.isEmpty(Ext.getCmp('consignee').getValue())){
						    	cotOrderFac.consignee =txtAreaAry.consignee;
						    }else{
						    	cotOrderFac.consignee = Ext.getCmp('consignee').getValue();
						    }
						...
						cotOrderFac.qcLocation=Ext.getCmp('qcLocation').getValue();

						cotOrderFac = setOrderFac(cotOrderFac);
						// printObject(cotOrderFac);
						cotOrderFacService.saveOrUpdateOrderFac(cotOrderFac,
								$('orderTime').value, $('sendTime').value,$('shipmentDate').value, $('addTime').value,checkFlag, function(res) {
									if (res != null) {
										if ($('pId').value =='' || $('pId').value== 'null') {
										...
										ds.save();
										// 调用存储过程，更新单据状态
										cotOrderFacService.updateOrderStatusCon(
												res, $("orderId").value,
												function(code) {
													reloadHome();
												});
	...
	function qingShen() {
		Ext.MessageBox.confirm('Message',
				"Do you want Approval?", function(btn) {
					if (btn == 'yes') {
				...
				function(btn) {
					if (btn == 'yes') {
						...
						var task = new Ext.util.DelayedTask(function(){
							DWREngine.setAsync(false);
							cotOrderFacService.updateOrderStatus($('pId').value, 2,
									null, function(res) {
										orderStatusBox.setValue(2);
	...
	function sendMail(){
		...
		mask();
		var task = new Ext.util.DelayedTask(function(){
			contexUtil.sendMail($('orderNo').value,"orderfac",custMail,function(res){
				if(res){
					unmask();
